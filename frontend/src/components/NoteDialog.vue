<template>
  <div :class="{ 'tg-theme': isTelegram }" :data-theme="isDarkTheme ? 'dark' : 'light'" v-if="noteDialog.isOpen"
    class="note-dialog-overlay" @click.self="closeDialog">
    <div class="note-dialog">
      ...
      <textarea v-model="currentNote.content" class="dialog-textarea" placeholder="Введите текст заметки..."
        ref="textarea" @click="focusTextarea"></textarea>
...
export default {
  ...
  methods: {
    ...
    async saveNote() {
      try {
        if (this.currentNote?.id) {
          // Обновляем заметку с актуальными данными
          const updatedNote = {
            ...this.currentNote,
          ...
          await this.$store.dispatch('fetchAllDataLists');
        }
      } catch (error) {
    ...
    async deleteNote() {
      try {
        if (this.currentNote?.id) {
          await this.$store.dispatch('deleteNote', this.currentNote.id);
          this.closeDialog();
  ...
  watch: {
    'noteDialog.isOpen'(newVal) {
      this.$nextTick(() => {
        // Форсируем обновление данных при открытии
